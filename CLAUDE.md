...
Stand: v0.11.0. Web-Tool, mit dem ein DM eine Rätsel-Oneshot leitet. **Zwei-iPad-Modell**:
...
- Zentral: `session.stage = { mode, payload }`. DM schaltet via `dm:set_stage`; Server broadcastet `stage_update`. Modi: `cover`, `puzzle`, `narration`, `crystals`, `image`.
...
## Nächster Schritt
Etage 5 – Sanctum (Mira-Konfrontation + Aldric-Fragen + finale Entscheidung) als eigene
Bühne(n) ausbauen: (1) Story-Events mit Verzweigungen → (2) interaktives Element / Abstimmung
auf dem Display + Server-Logik → (3) DM-Steuerung im „Anzeige"-Tab.

Offen/optional: gemeinsame „Gruppen-Funde"-Leiste auf dem Display (Ersatz fürs frühere private
Inventar), falls gewünscht.
