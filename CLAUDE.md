...
Stand: v0.18.0. Web-Tool, mit dem ein DM eine Rätsel-Oneshot leitet. **Zwei-iPad-Modell**:
...
- `client/src/components/dm/Cockpit.jsx` – die eine Steuerzentrale: **Live-Statusleiste** (was zeigt das Display + Code + Timer-Indikator + Titelbild-Reset) und ein mehrspaltiges Board (`columnWidth`) mit allen Blöcken: Bühne, Karten, Audio, Etagen & Rätsel, Ereignisse, Nachrichten, Funde, Timer.
...
## Nächster Schritt
Komplett: alle Etagen, Finale, Gruppen-Funde, Karten-Upload (mit Verwaltung), Sound-Upload. Mögliche Ausbauten:
- Karten-/Bild-Material fest ins Adventure-JSON hinterlegen (Velmoor-Stadtplan etc.) statt nur Upload.
