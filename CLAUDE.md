...
Stand: v0.19.0. Web-Tool, mit dem ein DM eine Rätsel-Oneshot leitet. **Zwei-iPad-Modell**:
...
- `client/src/views/DMView.jsx` – DM-Ansicht, Tabs: **Cockpit** (alles), **Finale** (nur wenn Adventure einen `konfrontation`-Floor hat), **Adventure**.
- `client/src/components/dm/Cockpit.jsx` – die eine Steuerzentrale: **Live-Statusleiste** (was zeigt das Display + Code + Timer-Indikator + Titelbild-Reset), das **Regie-Panel** (oben, volle Breite) und ein **festes 3-Spalten-Board** (CSS Grid, kein Umfließen): Spalte 1 Display steuern (Bühne/Karten/Audio), Spalte 2 Spielverlauf (Etagen & Rätsel/Ereignisse), Spalte 3 Kommunikation (Nachrichten/Funde/Timer).
- `client/src/components/dm/DirectorPanel.jsx` – **Regie/Souffleur**: zeigt zur aktuellen Etage `dmGuide` aus dem Adventure (Aufhänger zum Sagen, Szene/Atmosphäre, Ziel, ausklappbar Welt-Details + Lösung). Wechselt automatisch mit `currentFloor`.
- `client/src/components/dm/StagePanel.jsx` – exportiert die Cockpit-Bausteine: `SceneControls` (Bühnen-Umschaltung), `ImageManager` (Karten), `SoundManager` (Audio), `FindsManager` (Funde). Kein eigener Tab mehr.
...
- **Bilder/Karten**: DM lädt mehrere Bilder hoch (`POST /api/dm/image`, data-URL, mit optionalem Namen) oder gibt eine URL an. Rohdaten in `imageStore`, Metadaten `session.images` (broadcast `images_update`), Anzeige über `/api/image/:id`. `ImageManager` (StagePanel): Galerie mit Thumbnails – Klick zeigt (Bühne `image`), `dm:rename_image`/`dm:remove_image` zum Umbenennen/Löschen. Upload zeigt **nicht** automatisch. Löschen der gerade gezeigten Karte setzt die Bühne auf `cover` zurück.
...
- Jede Etage hat einen **`dmGuide`** (`{ say, scene, goal, details[], solution }`) → Regie-Panel im Cockpit.
...
## Design-Dokumente
`docs/` enthält die (ins Repo geretteten) Design-Docs: `app_konzept.md`, `konfrontation_mira.md`,
`konzept_TEILWEISE-WIEDERHERGESTELLT.md`, `velmoor_VERLOREN.md`. **Wichtig:** Außerhalb des Repos
liegende Dateien sind durch einen Umgebungs-Fehler schon verschwunden – Neues immer ins Repo committen.

## Nächster Schritt
Komplett: alle Etagen, Finale, Regie-Hilfe (`dmGuide`), Cockpit, Gruppen-Funde, Karten- & Sound-Upload. Mögliche Ausbauten:
- `dmGuide`-Inhalte verfeinern (besonders Welt-Details aus `velmoor.md`, das verloren ging).
